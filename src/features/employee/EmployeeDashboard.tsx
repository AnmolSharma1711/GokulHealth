    ...
    <div className="space-y-8 pb-28">
      ...
      <div className="glass p-8 rounded-3xl relative overflow-hidden animate-fade-in-up">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
          <div>
            <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Your Assignments</h2>
            <p className="text-slate-500 dark:text-slate-300 mt-1 text-lg">Currently managing <strong className="text-emerald-600 dark:text-emerald-400">{activeJobs.length}</strong> active jobs.</p>
          </div>
          <div className="flex items-center gap-2 text-emerald-700 dark:text-emerald-300 bg-emerald-100/50 dark:bg-emerald-900/30 px-5 py-2.5 rounded-2xl font-bold text-sm backdrop-blur-sm border border-emerald-200/50 dark:border-emerald-700/50 shadow-sm">
              ...
              <CardContent className="p-8">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="bg-emerald-50 dark:bg-emerald-900/40 p-3 rounded-2xl group-hover:scale-110 transition-transform duration-300">
                    <BriefcaseMedical className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
                  </div>
                </div>
                
                <div className="space-y-6 mt-6 pt-6 border-t border-slate-100/50 dark:border-slate-800/50 relative">
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700/50">
                      ...
                      <p className="text-sm font-medium text-slate-900 dark:text-white mt-1">{job.patient_age || 'N/A'}</p>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700/50">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Shift Timing</p>
                      <p className="text-sm font-medium text-slate-900 dark:text-white mt-1">{job.time_each_day || 'Not specified'}</p>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700/50">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Start Date</p>
                      <p className="text-sm font-medium text-slate-900 dark:text-white mt-1">{job.start_date || 'N/A'}</p>
                    </div>
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700/50">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">End Date</p>
                      <p className="text-sm font-medium text-slate-900 dark:text-white mt-1">{job.end_date || 'N/A'}</p>
                    </div>
                    {job.service_details && (
                      <div className="col-span-2 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700/50">
                        ...
                        <p className="text-sm font-medium text-slate-900 dark:text-white mt-1 italic">{job.service_details}</p>
                      </div>
                    )}
                  </div>
                  
                  <div className="flex items-start gap-4 mb-6">
                    <div className="bg-slate-50 dark:bg-slate-800/50 p-2 rounded-xl mt-0.5">
                      <MapPin className="w-5 h-5 text-slate-400" />
                    </div>
                    <div>
                      <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Customer Reference</p>
                      <p className="text-base font-medium text-slate-900 dark:text-white mt-1">{job.customer_id.substring(0, 8).toUpperCase()}</p>
                    </div>
                  </div>
